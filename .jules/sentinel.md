...
## 2024-05-24 - Information Exposure in Upload Script
**Vulnerability:** The `main/upload_db.php` script used `print_r($_FILES)` to output detailed error information directly to the user upon an upload failure. This could leak internal file paths, temporary filenames, and other structural details. Additionally, because the `name` attribute of the uploaded file is user-controlled, outputting it directly via `print_r` in an HTML context introduced a potential Reflected XSS vulnerability.
**Learning:** Development debugging functions like `print_r` or `var_dump` should never be left in production code where their output is visible to end-users, especially when handling user-controlled data like file uploads.
**Prevention:** Use server-side logging mechanisms like `error_log()` to record sensitive debugging information. Present only generic, safe error messages to the end-user (e.g., "An error occurred while processing the file").
