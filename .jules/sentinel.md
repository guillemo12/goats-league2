...
## 2024-05-10 - Insecure Session Management
**Vulnerability:** Found two critical session vulnerabilities: Session Cookies lacked `HttpOnly` and `SameSite=Lax` attributes, making them vulnerable to XSS and CSRF. Furthermore, the login endpoint (`main/login/auth.php`) failed to regenerate the session ID upon successful authentication, leaving the application vulnerable to Session Fixation attacks.
**Learning:** Default PHP session configurations are often insecure. Simply setting a lifetime and path is insufficient. `session_regenerate_id(true)` is a necessary and standard defense during state changes like authentication.
**Prevention:** Enforce strict session cookie attributes (`httponly => true`, `samesite => 'Lax'`) globally in all entry points. Always invoke `session_regenerate_id(true)` immediately after successful user authentication to assign a new, untainted session ID.
