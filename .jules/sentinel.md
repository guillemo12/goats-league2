...
## 2024-05-18 - [Path Disclosure and Insecure Permissions]
**Vulnerability:** Information disclosure via verbose error messages (exposing `$_FILES` with `print_r`) and raw paths (`getcwd()`, `$target_dir`) alongside overly permissive `0777`/`0666` file permissions in `main/upload_db.php`.
**Learning:** `chmod` and `mkdir` should use restrictive permissions (e.g. `0700` and `0600`) as the executing PHP process already runs as the owner and does not need to grant access to other system users. Verbose details are great for debugging but must be routed to `error_log()` rather than printed directly to the user interface.
**Prevention:** Avoid `print_r` or `var_dump` of system variables like `$_FILES` or `$_SERVER` in production views. Ensure directories are created with `0700` and files with `0600` permissions. Ensure cleanup of any patch/backup files to avoid source code disclosure.
