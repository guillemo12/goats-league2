...
## 2024-05-03 - Insecure Sessions and Session Fixation
**Vulnerability:** Session cookies lacked `httponly` and `samesite` protection, leaving them vulnerable to XSS and CSRF attacks. Furthermore, the application didn't regenerate the session ID upon successful login, exposing it to Session Fixation attacks.
**Learning:** In applications using decentralized session initialization (like calling `session_start()` in every file), session parameters must be consistently enforced across all endpoints. Also, regenerating session IDs on privilege changes (like login) is a fundamental defense against session hijacking.
**Prevention:** Centralize session initialization logic if possible, or systematically apply secure cookie flags (`httponly`, `samesite`, and ideally `secure`) to all `session_set_cookie_params` calls. Always use `session_regenerate_id(true)` immediately after successful authentication.
