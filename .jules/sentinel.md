...
## 2024-04-16 - Session Fixation Vulnerability
**Vulnerability:** The login authentication flow in `main/login/auth.php` did not regenerate the session ID upon successful login, leading to a potential Session Fixation vulnerability where an attacker could hijack a user's session.
**Learning:** Preventing Session Fixation in PHP requires regenerating the session ID when privilege escalation occurs (like successfully logging in). Also, avoid session-based rate limiting as it's easily bypassed (security theater).
**Prevention:** Always use `session_regenerate_id(true);` upon successful authentication to invalidate the old session and create a new secure session ID.
