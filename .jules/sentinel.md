...
## $(date +%Y-%m-%d) - Fix Insecure Database Upload Permissions and Info Disclosure
**Vulnerability:** The script `main/upload_db.php` was creating directories with `0777` permissions and uploaded `.sqlite` files with `0666` permissions, allowing world-writable/executable access. Additionally, it lacked extension validation, and upon upload failure, it printed the raw `$_FILES` array to the user, leaking the server's temporary directory structure via the `tmp_name` property.
**Learning:** File upload endpoints handling sensitive database files must enforce restrictive permissions (least privilege) and avoid using debug functions like `print_r()` on sensitive variables directly to the user to prevent information disclosure. Input validation on extensions is critical to ensure only expected file types are uploaded to specific directories.
**Prevention:** Enforce strict file (`0600`) and directory (`0700`) permissions upon creation. Always log detailed debug information like `$_FILES` to `error_log()` instead of echoing it to the client. Validate file extensions strictly before attempting `move_uploaded_file()`.
