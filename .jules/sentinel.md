...
## 2024-08-24 - [Information Disclosure & Insecure Permissions] DB Upload Script
**Vulnerability:** The DB upload script (`main/upload_db.php`) created directories with `0777` permissions and set the uploaded SQLite file to `0666`. It also lacked file extension validation and output `print_r($_FILES)` directly to the user on error, disclosing sensitive internal file paths and metadata.
**Learning:** Even internal admin tools must enforce least privilege (`0700` dirs, `0600` files for SQLite databases meant to be read only by the web server process), validate file types strictly, and handle debugging information securely.
**Prevention:** Always use restrictive permissions (like `0600` or `0640` for files, `0700` or `0750` for directories). Ensure `.sqlite` validation for DB uploads, and log errors securely using `error_log()` instead of direct string output to users.
