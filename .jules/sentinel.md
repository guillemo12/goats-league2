...
## 2024-05-24 - [Insecure Database Upload]
**Vulnerability:** The database upload script (`main/upload_db.php`) possessed multiple critical vulnerabilities: it configured excessive directory (`0777`) and file (`0666`) permissions, exposed sensitive debug data (`print_r($_FILES)`) to end-users, accepted arbitrary file extensions without validation, and failed to sanitize output variables resulting in reflected XSS.
**Learning:** These types of utility scripts are often implemented quickly as internal admin tools and lack fundamental security checks. Specifically, granting public write access to critical database stores via permissive `chmod` allows for database tampering. Further, returning unmodified `$_FILES` payloads directly to the client leaks the underlying file system structure (like `/tmp/...`).
**Prevention:** Always restrict data stores (like SQLite databases) to owner-only permissions (`0600`/`0700`). Implement robust input validation even for authenticated admin endpoints (e.g., verifying `.sqlite` or `.db` extensions). Avoid outputting internal debug structures on failure; log them via `error_log` instead. Wrap variables passed to the DOM in `htmlspecialchars()`.
