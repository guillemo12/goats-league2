...
## 2026-07-30 - Fix DOM-based XSS in JavaScript DOM Updates
**Vulnerability:** DOM-based XSS vulnerabilities existed in `match.php` and `tratos.php`. Untrusted user data (like `data.error` messages from the server and usernames from the database) was being dynamically rendered in the frontend via unescaped string concatenation directly into the `.innerHTML` property.
**Learning:** Developers frequently overlook DOM-based XSS when data originates 'safely' from their own backend APIs, assuming it's pre-sanitized or inherently trustworthy. However, API response strings must still be treated as untrusted context when placed into HTML execution sinks.
**Prevention:** Always construct dynamic DOM nodes programmatically using `document.createElement()` and append text containing user or external data using `document.createTextNode()` or `.textContent`. Never use `.innerHTML` combined with string interpolation for dynamic variables.
