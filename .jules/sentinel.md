...
## 2024-05-18 - Server-Side & DOM-Based Stored XSS in Market Trades
**Vulnerability:** Found multiple Stored XSS vulnerabilities in `main/tratos.php`. First, a server-side XSS existed in `getPlayerNames()`, which joined usernames directly from the database and echoed them without escaping. Second, a DOM-based XSS existed where a JS fetch request interpolated `p.username` directly into an element's `.innerHTML`.
**Learning:** Functions that aggregate database strings (like a comma-separated list of names) must apply escaping to each element before joining them, because the caller will likely just echo the entire list directly into HTML. In JavaScript, dynamically constructed HTML must use safe DOM APIs (`document.createElement` and `textContent`) instead of `.innerHTML` when handling user data.
**Prevention:** Always apply `htmlspecialchars()` to strings before outputting them in PHP or use `array_map('htmlspecialchars', $array)` before joining arrays for output. In JavaScript, always use `textContent` to inject user-provided text, and configure `json_encode` with `JSON_HEX_*` security flags when passing PHP data to JS.
