...
## 2024-05-14 - DOM-based Stored XSS via .innerHTML
**Vulnerability:** Found multiple instances where database-sourced user input (e.g. usernames from `/main/tratos.php`) and server error messages (from `/main/match.php`) were assigned directly to elements using `.innerHTML`. This allowed XSS payloads stored in the database to execute on the victim's browser when rendering UI components asynchronously.
**Learning:** Never trust data originating from JSON responses when manipulating the DOM. Using string concatenation or template literals injected via `.innerHTML` directly translates to DOM XSS.
**Prevention:** Avoid `.innerHTML` entirely when handling unescaped user input or API responses. Use secure DOM construction methods like `document.createElement()`, `document.createTextNode()`, and `Element.textContent` to ensure the browser treats the input strictly as data, not executable HTML.
