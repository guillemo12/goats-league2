...
## 2024-05-24 - [Information Disclosure in Upload File]
**Vulnerability:** Information Disclosure and Arbitrary File Upload via `main/upload_db.php`. Permissive directory creation (`0777`) and file upload handling (`0666`), combined with a lack of file extension validation, allowed arbitrary files to be uploaded with weak permissions. In case of upload failures, `print_r($_FILES)` exposed the internal temporary path structures and the outcome of an `is_writable` check on the server directories directly to the user.
**Learning:** Hardcoding open permissions (`0777`/`0666`) is a common anti-pattern that circumvents least-privilege principles. Debugging tools like `print_r` are frequently left in production code by accident, exposing sensitive internal state.
**Prevention:** Enforce strict file permissions (`0700` for directories, `0600` for sensitive files). Validate file extensions strictly on file upload endpoints. Log debugging information securely server-side via `error_log()` instead of echoing it to the frontend.
