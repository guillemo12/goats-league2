...
## 2024-05-24 - [Fix DOM XSS vulnerabilities in UI]
 **Vulnerability:** DOM-based Cross-Site Scripting (XSS) via `innerHTML` injection of dynamically loaded JSON API response error strings (`data.error` in `main/match.php`) and username interpolation (`p.username` in `main/tratos.php`).
 **Learning:** Vanilla JavaScript patterns using string interpolation and `.innerHTML` concatenated with untrusted external API data introduce significant XSS vectors, specifically because DOM-manipulation directly writes unsanitized payloads into the browser.
 **Prevention:** Prevent DOM-based XSS when creating dynamic UI elements by exclusively using safe DOM assignment properties like `.textContent` or `document.createTextNode()` instead of `.innerHTML` for any dynamic text, which guarantees the browser treats the input strictly as a string literal instead of evaluable HTML.
