...
**Vulnerability:** User input and untrusted API JSON responses containing dynamic information (like `data.error` or player `username`) were being directly interpolated into `.innerHTML` strings in client-side JavaScript, leading to potential DOM-based XSS vulnerabilities.
**Learning:** Developers may casually concatenate variables in template literals without considering the source of the data. Untrusted data originating from JSON API responses (or user-controlled database fields) can trigger XSS when rendered dynamically without proper escaping in the DOM.
**Prevention:** Avoid `.innerHTML` for any string that incorporates untrusted data. Instead, construct elements using `document.createElement()` and assign untrusted values via `textContent`, or use `document.createTextNode()` to append strings securely to the DOM.
