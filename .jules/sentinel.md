...
## 2024-05-24 - [Fix DOM-based XSS in tratos.php and match.php]
**Vulnerability:** DOM-based XSS. Unsanitized strings were being assigned to `.innerHTML` in `main/tratos.php` (`p.username`) and `main/match.php` (`data.error`).
**Learning:** Using `.innerHTML` to insert dynamic, potentially user-controlled content is a common source of DOM-based XSS.
**Prevention:** Always use safe DOM manipulation methods like `document.createElement()` and `textContent` or `document.createTextNode()` when inserting dynamic text content into the DOM, avoiding string interpolation with `.innerHTML` for user-supplied data.
