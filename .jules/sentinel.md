...
## 2024-05-14 - DOM XSS via innerHTML
**Vulnerability:** A DOM-based Cross-Site Scripting (XSS) vulnerability was found in `main/tratos.php`. The application fetched user data (player names) via AJAX and injected it directly into the DOM using template literals and `.innerHTML` (e.g., `div.innerHTML = "... ${p.username} ...";`).
**Learning:** Even when data is retrieved from your own backend API, if it originates from user input (like usernames) and is not strictly validated, inserting it into the DOM via `.innerHTML` creates an XSS vulnerability on the client side.
**Prevention:** Always use safe DOM manipulation methods in JavaScript when handling dynamic or user-supplied data. Specifically, use `document.createElement()` to build elements and assign text content using `.textContent` or `.innerText` to ensure HTML characters are safely escaped by the browser before rendering.
