...
if (isset($_POST["submit"])) {
    $file_extension = strtolower(pathinfo($_FILES["db_file"]["name"], PATHINFO_EXTENSION));
    if ($file_extension !== "sqlite") {
        echo "❌ Error: Solo se permiten archivos .sqlite.";
    } elseif (move_uploaded_file($_FILES["db_file"]["tmp_name"], $target_file)) {
        chmod($target_file, 0600); // Permiso de lectura/escritura para la DB
        echo "✅ ¡Éxito! Archivo subido a: " . $target_file;
    }
    else {
        echo "❌ Error al subir. Revise los registros del sistema.";
        error_log("DB Upload Error: " . print_r($_FILES, true));
