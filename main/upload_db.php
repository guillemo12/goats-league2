...
if (isset($_POST["submit"])) {
    $file_ext = strtolower(pathinfo($_FILES["db_file"]["name"], PATHINFO_EXTENSION));

    if ($file_ext !== "sqlite") {
        echo "❌ Error al subir. Solo se permiten archivos .sqlite.";
    } else {
        if (move_uploaded_file($_FILES["db_file"]["tmp_name"], $target_file)) {
            ...
            echo "❌ Error al subir. Inténtelo de nuevo o revise los registros del sistema.";
            error_log("Error al subir archivo de DB. Detalles: " . print_r($_FILES, true) . ". ¿La carpeta es escribible?: " . (is_writable($target_dir) ? 'SÍ' : 'NO'));
        }
    }
}
?>
