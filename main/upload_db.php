...
if (isset($_POST["submit"])) {
    ...
    if ($file_ext !== 'sqlite') {
        echo "❌ Error: El archivo debe tener la extensión .sqlite";
    }
    else if (move_uploaded_file($_FILES["db_file"]["tmp_name"], $target_file)) {
        chmod($target_file, 0600); // Permiso de lectura/escritura para la DB
        echo "✅ ¡Éxito! Archivo subido a: " . $target_file;
    }
    else {
        echo "❌ Error al subir el archivo.";
        error_log("Error al subir DB. Detalles: " . print_r($_FILES, true) . ". Carpeta escribible: " . (is_writable($target_dir) ? 'SÍ' : 'NO'));
    }
}
?>
