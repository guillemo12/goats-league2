...
if (isset($_POST["submit"])) {
    if (move_uploaded_file($_FILES["db_file"]["tmp_name"], $target_file)) {
        chmod($target_file, 0600); // Permiso de lectura/escritura para la DB (solo usuario local)
        echo "✅ ¡Éxito! Archivo subido a: " . $target_file;
    }
    else {
        error_log(print_r($_FILES, true));
        echo "❌ Error al subir la base de datos. Por favor, inténtelo de nuevo más tarde.";
